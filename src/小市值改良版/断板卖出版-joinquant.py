...
def initialize(context):
    ...
    g.today_HL_list = []  # 记录今日涨停的股票
    g.today_HL_remove_list = []  # 记录今日涨停破板减仓的股票
    #全局变量float/str
    g.min_mv = 1  # 股票最小市值要求
    ...
    g.stoploss_limit = 0.5  # 止损线
    g.stoploss_market = 0.04  # 市场趋势止损参数
    g.highest = 50  # 股票单价上限设置
    g.etf = '511880.XSHG'  # 空仓月份持有银华日利ETF
    # 设置交易运行时间
    run_daily(prepare_stock_list, '9:05')
    run_daily(trade_afternoon, time='14:00', reference_security=BLOCK) #检查持仓中的涨停股是否需要卖出
    run_daily(sell_stocks, time='10:00') # 止损函数
    run_daily(close_account, '14:50')
    run_weekly(weekly_adjustment, 4,'10:00')
...
def get_stock_list(context):
    ...
    ).order_by(valuation.market_cap.asc()).limit(g.stock_num*5)
...
def handle_data(context, data):
    ...
    for stock in hold_list:
        ...
        if current_data[stock].last_price == current_data[stock].high_limit:
            ...
            if stock in g.today_HL_list and stock not in g.today_HL_remove_list and stock in g.yesterday_HL_list:
                g.today_HL_remove_list.append(stock)
                log.info(f'{stock}已涨停破板，加入破板列表')
                # 卖出一半
                close_position(context.portfolio.positions[stock], 1)
                
# 昨日涨停板应当有更早的卖点：涨停破板 & 放巨量
# def check_limit_up_pre(context):
#     print('涨停破板卖出提前批')
...
def check_remain_amount(context):
    if g.reason_to_sell is 'limitup': #判断提前售出原因，如果是涨停售出则次日再次交易，如果是止损售出则不交易
        ...
        if len(g.hold_list) < g.stock_num:
            ...
            log.info('有余额可用'+str(round((context.portfolio.cash),2))+'元。买入'+ str(get_stock_name_of_list(target_list)))
            buy_security(context,target_list)
        g.reason_to_sell = ''
    elif g.reason_to_sell is 'stoploss':
        log.info('有余额可用'+str(round((context.portfolio.cash),2))+'元。买入ETF')
        buy_security(context,[g.etf])
        g.reason_to_sell = ''
        ...
        if g.stoploss_strategy == 1 or g.stoploss_strategy == 3:
            for stock in current_positions.keys():
                ...
                if price >= avg_cost * 1.5:
                    ...
                    log.debug("收益100%止盈,卖出{}".format(get_stock_name(stock)))
                # 个股止损
                elif price < avg_cost * (1 - g.stoploss_limit):
                    order_target_value(stock, 0)
                    log.debug("收益止损,卖出{}".format(get_stock_name(stock)))
                    g.reason_to_sell = 'stoploss'

        if g.stoploss_strategy == 2 or g.stoploss_strategy == 3:
            stock_df = get_price(security=get_index_stocks(BLOCK), end_date=context.previous_date, frequency='daily', fields=['close', 'open'], count=1, panel=False)
...
def adjust_stock_num(context):
    ...
    index_df = get_price(BLOCK, start_date=start_date, end_date=today, frequency='daily')
...
def filter_stocks(context, stock_list):
    ...
    print(f"初选数量{len(filtered_stocks)}")
...
#3-3 交易模块-平仓
def close_position(position, type = 0):
    security = position.security
    if type == 0:
      order = order_target_value_(security, 0)  # 可能会因停牌失败
    else:
      order = order_target_value_(security, 0)  # 可能会因停牌失败
      # order = order_target_value_(security, position.value / 2)
    if order != None:
...
def get_stock_name_of_list(code_list):
    value = ''
    for code in code_list:
        stock_info = get_security_info(code)
        value += ',' + stock_info.display_name
...
def print_position_info(context):
    ...
    for position in list(context.portfolio.positions.values()):
        ...
        print('股票:{}'.format(get_stock_name(securities)))
