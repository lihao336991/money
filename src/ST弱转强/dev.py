...
def prepare(ContextInfo):
    ...
    if GJTFliterStockList:
        ...
        # fStockList = filter_stocks_new(ContextInfo, GJTFliterStockList)
    else:
        print("\n国九条筛选后无股票，跳过技术筛选")
...
def filter_stocks_new(ContextInfo, stocks):
    ...
    df = ContextInfo.get_market_data_ex(
        ...
        stock_code=stocks,
        period='1d',
        start_time = (ContextInfo.today - datetime.timedelta(days=20)).strftime('%Y%m%d'),
        end_time = (ContextInfo.today - datetime.timedelta(days=1)).strftime('%Y%m%d'),
        count=11, # 昨天往前选11根日k线数据，10根用于计算MA10，1根用于shift
        fill_data=False,
        subscribe=False
    )
    if df.empty:
    ...
    df['low_limit'] = df['prev_close'] * 0.95  # 计算跌停价
    ...
    # 原版未启用
    # pct_series = grouped['low'].pct_change()*100   # 今日相对昨日最低价的变化率
    # prev_close = pct_series.iloc[-1] # BUG: 变量名prev_close与之前的列名冲突，且逻辑存在问题，予以删除
    # 构建筛选条件
    conditions = (
    ...
    # 获取每个分组的最后一条数据作为最新数据，比按日期过滤更稳健
    latest_data = grouped.tail(1)
