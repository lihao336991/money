...
def initialize(context):
    ...
    print("初始函数开始运行且全局只运行一次")  # 输出内容到日志 print()
...
def update_sector_pool(context):
    ...
    exclude_keywords = ['300', '500', '1000', '50', '上证', '创业板', '科创', '恒生', 'H股', '货币', '纳指', '标普', '债', '短融']
    ...
    def filter(self):
        ...
        for etf in self.last_etf + g.dynamic_etf_pool:
            ...
            if etf in g.dynamic_etf_pool:
                data.loc[etf, "score"] -= 1 # 仅当动态池子分高于动态池子超过1分，才有可能被选中
                # data.loc[etf, "score"] *= 0.6

            # 过滤近3日跌幅超过5%的ETF
            if min(prices[-1] / prices[-2], prices[-2] / prices[-3], prices[-3] / prices[-4]) < 0.95:
                data.loc[etf, "score"] = 0
            # 过滤近2日涨幅超过9%的ETF（乖离太大，避免极端行情追高）
            if max(prices[-2] / prices[-3], prices[-3] / prices[-4]) > 1.09:
                data.loc[etf, "score"] = 0

        # 过滤ETF，并按得分降序排列
        data = data.query("0.2 < score < 6").sort_values(by="score", ascending=False)
        print(data)
