...
def get_stock_list(context): 
    ...
    # 情绪退潮判断（昨日因子下降超过10%）
    if last_high_limit_factor == 0 or yesterday_high_limit_factor / last_high_limit_factor < 0.9 and yesterday_high_limit_factor < 0:
        g.gap_up = []
        message = f"涨停情绪退潮，今日空仓，昨日涨停因子：{yesterday_high_limit_factor:.2f}，前日涨停因子：{last_high_limit_factor:.2f}"
        log.info(message)
...
def buy(context):
    ...
    # 1. 筛选符合条件的股票
    for s in g.gap_up:
        prev_day_data = attribute_history(s, 1, '1d', fields=['close', 'volume'], skip_paused=True)
        # --- 修复点：fields 必须包含 'time' ---
        auction_data = get_call_auction(s, start_date=date_now, end_date=date_now, fields=['time', 'volume', 'current'])
        
        if auction_data.empty or prev_day_data.empty:
            continue
            
        # 集合竞价成交量需大于昨日成交量4%
        if auction_data['volume'][0] / prev_day_data['volume'][-1] < 0.04:
            continue

        # 筛选开盘涨幅在0-6%之间 (1.0表示平开，1.06表示涨6%)
        current_ratio = auction_data['current'][0] / (current_data[s].high_limit / 1.1)
        if 1.0 < current_ratio < 1.06:
            qualified_stocks.append(s)
            
    # 2. 对称仓位管理：最多持有4只，每只占总资产 25%
    max_count = 4 
    current_holdings = len(context.portfolio.positions)
    available_slots = max_count - current_holdings

    if available_slots <= 0 or not qualified_stocks:
        return

    # --- 核心改进：按竞价成交金额从大到小排序 ---
    qualified_stocks.sort(key=lambda x: auction_data['current'][0] * auction_data['volume'][0], reverse=True)
    
    # 取前 N 名
    buy_list = [item for item in qualified_stocks[:available_slots]]

    # 计算每只股票的目标价值（总资产的 1/4）
    target_value_per_stock = context.portfolio.total_value / max_count

    for s in buy_list:
        # 只有在可用资金充足时才下单
        # 考虑到手续费和滑点，实际下单金额稍微打个 99% 的折比较稳妥
        if context.portfolio.available_cash > target_value_per_stock:
            order_target_value(s, target_value_per_stock * 0.99)
            print(f"【买入信号】股票: {s}, 目标持仓权重: 25%")
        else:
            print(f"【资金不足】尝试买入 {s}，但可用资金不足以支撑 25% 仓位")
