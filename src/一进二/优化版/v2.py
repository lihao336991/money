...
# 纯一进二优化版 v2
# 参数对齐：参考 src/三合一/优化版/v2.py 中的一进二部分
...
def initialize(context):
    ...
    # 设置每日定时任务
    run_daily(get_stock_list, '9:01')  # 9:01获取股票列表
    run_daily(buy, '09:26')           # 9:26执行买入
    # 设置两个卖出时间点
...
def get_stock_list(context): 
    ...
    # 对齐三合一版本：一进二部分不再使用情绪退潮直接空仓过滤
    g.gap_up = get_priority_list(context, hl_list, date)
    log.info('一进二初选：昨日涨停%s只，T-1曾涨停%s只，T-2曾涨停%s只，候选%s只' % (
        len(hl0_list), len(hl1_list), len(hl2_list), len(g.gap_up)
    ))
    return
...
def buy(context):
    ...
    for s in g.gap_up:
        ...
        # 对齐三合一版本：筛选集合竞价成交量大于前一日成交量3%的股票
        if auction_data.empty or auction_data['volume'][0] / prev_day_data['volume'][0] < 0.03:
            continue
