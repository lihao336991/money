...
# 纯一进二回测版：集合竞价确认后延迟5分钟成交
# 用途：测试开盘瞬间无法成交、9:35再成交，并排除开盘直线拉高个股的表现
...
DELAYED_BUY_TIME = '09:35'
OPEN_OBSERVE_START = ' 09:30:00'
OPEN_OBSERVE_END = ' 09:35:00'

# 直线拉高过滤参数，可按回测结果微调
MAX_5MIN_GAIN_FROM_OPEN = 0.03        # 9:35相对开盘价涨幅超过3%，认为开盘直线拉高
MAX_5MIN_GAIN_FROM_PREV_CLOSE = 0.06  # 9:35相对昨收涨幅超过6%，不再追入
SKIP_TOUCHED_HIGH_LIMIT = True        # 前5分钟触及涨停，认为基本不可成交
...
def initialize(context):
    ...
    run_daily(buy, DELAYED_BUY_TIME)     # 延迟到9:35执行买入
    # 设置两个卖出时间点
...
def buy(context):
    ...
    # 先用集合竞价条件确认目标，再用开盘后5分钟走势决定是否延迟成交
    start = date_now + OPEN_OBSERVE_START
    end = date_now + OPEN_OBSERVE_END

    # 遍历"一进二"股票池
    for s in g.gap_up:
        # 获取前一日数据
        prev_day_data = attribute_history(s, 1, '1d', fields=['close', 'volume', 'money'], skip_paused=True)
        if prev_day_data.empty or prev_day_data['volume'][-1] <= 0 or prev_day_data['close'][-1] <= 0:
            continue
        ...
        delayed_price = get_delayed_buy_price(s, start, end)
        if delayed_price is None:
            continue

        if is_open_straight_pull_up(
            s,
            start,
            end,
        ...
        # 添加到候选列表
        qualified_stocks.append((s, delayed_price))
    ...
    if len(qualified_stocks) != 0 and context.portfolio.available_cash / context.portfolio.total_value > 0.3:
        ...
        for s, delayed_price in qualified_stocks:
            # 确保有足够资金买入至少100股
            if delayed_price > 0 and context.portfolio.available_cash / delayed_price > 100:
                order_value(s, value)
                print('延迟5分钟买入', s, '参考价', delayed_price)
                print('———————————————————————————————————')


def get_open_5min_data(stock, start, end):
    """
    获取开盘后前5分钟分钟线。
    聚宽分钟线在不同环境可能返回空值，所以统一做防御。
    """
    df = get_price(
        stock,
        start_date=start,
...
def get_delayed_buy_price(stock, start, end):
    """
    用9:35附近最后一根分钟线收盘价模拟延迟成交价格。
    """
    df = get_open_5min_data(stock, start, end)
    if df is None or df.empty:
        return None
    return df['close'].iloc[-1]


def is_open_straight_pull_up(stock, start, end, prev_close, auction_price, high_limit):
    """
    排除开盘5分钟直线拉高、触及涨停或已经涨幅过大的股票。
    """
    df = get_open_5min_data(stock, start, end)
    if df is None or df.empty:
        return True

    delayed_price = df['close'].iloc[-1]
    first_open = df['open'].iloc[0]
    high_5min = df['high'].max()

    if first_open <= 0 or prev_close <= 0:
        return True

    if SKIP_TOUCHED_HIGH_LIMIT and high_limit > 0 and high_5min >= high_limit:
        return True

    if delayed_price / first_open - 1 > MAX_5MIN_GAIN_FROM_OPEN:
        return True

    if delayed_price / prev_close - 1 > MAX_5MIN_GAIN_FROM_PREV_CLOSE:
        return True

    if auction_price > 0 and delayed_price / auction_price - 1 > MAX_5MIN_GAIN_FROM_OPEN:
        return True
