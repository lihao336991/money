...
# 纯一进二回测版：集合竞价容量约束
# 单票买入金额 <= min(集合竞价成交额 * 5%, 昨日成交额 * 0.2%)
...
AUCTION_AMOUNT_PARTICIPATION_RATE = 0.05
PREV_DAY_AMOUNT_PARTICIPATION_RATE = 0.002
...
def buy(context):
    ...
    :param context: 策略上下文对象
    """
    qualified_stocks = []  # 符合条件的股票列表，元素为(stock, capacity_value)
    gk_stocks = []        # 高开股票列表
    ...
    for s in g.gap_up:
        # 获取前一日数据
        prev_day_data = attribute_history(s, 1, '1d', fields=['close', 'volume', 'money'], skip_paused=True)
        if prev_day_data.empty or prev_day_data['volume'][0] <= 0 or prev_day_data['money'][0] <= 0:
            continue

        # 获取集合竞价数据
        auction_data = get_call_auction(s, start_date=date_now, end_date=date_now, fields=['time','volume', 'current'])
        # 筛选集合竞价成交量大于前一日成交量4%的股票
        if auction_data.empty or auction_data['volume'][0] / prev_day_data['volume'][0] < 0.04:
            continue

        # 计算当前价格相对于涨停板价格的比率
        if current_data[s].high_limit <= 0:
            continue
        ...
        capacity_value = get_single_stock_capacity_value(
        ...
        # 添加到候选列表
        qualified_stocks.append((s, capacity_value))

    # 执行买入操作
    if len(qualified_stocks) != 0 and context.portfolio.available_cash / context.portfolio.total_value > 0.3:
        # 计算每只股票分配的资金
        target_value = context.portfolio.available_cash / len(qualified_stocks)
        for s, capacity_value in qualified_stocks:
            value = min(target_value, capacity_value, context.portfolio.available_cash)
            # 确保有足够资金买入至少100股
            if current_data[s].last_price > 0 and value / current_data[s].last_price > 100:
                order_value(s, value, MarketOrderStyle(current_data[s].day_open))
                print('容量约束买入', s, '目标金额', target_value, '容量上限', capacity_value, '实际金额', value)
                print('———————————————————————————————————')


def get_single_stock_capacity_value(auction_price, auction_volume, prev_day_money):
    """
    集合竞价容量约束：
    单票买入金额 <= min(集合竞价成交额 * 5%, 昨日成交额 * 0.2%)
    """
    if auction_price <= 0 or auction_volume <= 0 or prev_day_money <= 0:
        return 0

    auction_amount = auction_price * auction_volume
    auction_capacity = auction_amount * AUCTION_AMOUNT_PARTICIPATION_RATE
    prev_day_capacity = prev_day_money * PREV_DAY_AMOUNT_PARTICIPATION_RATE
    return min(auction_capacity, prev_day_capacity)
